...
i2c_master_dev_handle_t dev_handle_mpu6050;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    ESP_ERROR_CHECK(i2c_master_bus_add_device(bus_handle,&mpu6050_cfg,&dev_handle_mpu6050));
...
esp_err_t mpu6050_read_register(uint8_t reg_addr,uint8_t *reg,size_t length)
{
	esp_err_t res = i2c_master_transmit(dev_handle_mpu6050,&reg_addr,1,I2C_TICKS_TO_WAIT);
	if(res != ESP_OK)
	{
		ESP_LOGE(TAG,"Failed to send register address: %d (%s)",res,esp_err_to_name(res));
		return res;
	...
	res = i2c_master_receive(dev_handle_mpu6050,reg,length,I2C_TICKS_TO_WAIT);
	if(res != ESP_OK)
	{
		ESP_LOGE(TAG,"Failed to read data %d (%s)",res, esp_err_to_name(res));
	}
...
{
	uint8_t data[1];
	esp_err_t res = mpu6050_read_register(who_am_i_mpu6050,data,1);
	if(res!= ESP_OK)
	{
		ESP_LOGE(TAG, "Failed to get WHO AM I of MPU6050");
		return -1;
	}
	return data[0];
}
